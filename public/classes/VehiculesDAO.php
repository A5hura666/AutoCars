...
class VehiculesDAO extends DAO
{
    ...
    public function getOne(int|string $id): Vehicule
    {
        ...
        return new Vehicule($row['NoImmatriculation'], $row['CodeClient'], $row['Modèle'], $row['NoSerie'], $row['DateMiseEnCirculation'],$row['NumMarque']);
    }
    ...
    public function getAll(): array
    {
        ...
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row)
            $res[] = new Vehicule($row['NoImmatriculation'], $row['CodeClient'], $row['Modèle'], $row['NoSerie'], $row['DateMiseEnCirculation'],$row['NumMarque']);
        return $res;
    }
